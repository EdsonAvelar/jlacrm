                            ...
                            <div class="page-title-box">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    @foreach ($proprietarios as $proprietario_id => $value)
                                                        <a class="dropdown-item" target="_self"
                                                            href="{{ route('pipeline_index', ['id' => $curr_funil_id, 'proprietario' => $proprietario_id, 'view' => 'list', 'status' => 'ativo']) }}">{{ $value }}</a>
                                                    @endforeach
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end">
                                            @foreach ($funils as $id => $value)
                                                <a class="dropdown-item"
                                                    href="{{ route('pipeline_index', ['pipeline_id' => $id, 'view' => 'list']) }}">{{ $value }}</a>
                                            @endforeach
                                        </div>
                                    </div>
                                </div>
                                <h4 class="page-title">Negócios
                                    <a href="#" data-bs-toggle="modal" data-bs-target="#add-negocio-model"
                                        id="add_button" class="btn btn-info btn-sm ms-3">+ Add</a>
                                    ...
                                    @if (Auth::user()->hasRole('gerenciar_equipe'))
                                        <a type="button" class="btn btn-success btn-sm ms-3 checkbox_sensitive"
                                            id="ativar_btn" data-bs-toggle="modal" data-bs-target="#ativarModal">
                                            Ativar</a>
                                    @endif
                                ...
                                <tbody>
                                    @if (isset($negocios))
                                        @foreach ($negocios as $negocio)
                                            <tr>
                                                <td><input type="checkbox" id="chkBSA" name="negocios[]"
                                                        value="{{ $negocio->id }}" class="select-checkbox"></td>
                                                <td><a
                                                        href="{{ route('negocio_edit', ['id' => $negocio->id]) }}">{{ $negocio->titulo }}</a>
                                                </td>
                                                <td>{{ $negocio->lead->nome }}</td>
                                                <td><a
                                                        href="tel:{{ $negocio->lead->telefone }}">{{ $negocio->lead->telefone }}</a>
                                                </td>
                                                <td>{{ $negocio->valor }}</td>
                                                <td>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer ">
                        ...
                        <input type="button" class="btn btn-danger mt-2 atribuir" data-bs-dismiss="modal"
                            value="Cancelar">
                    </div>
                </div>
            </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-4 scroll">
                                ...
                                @foreach ($users as $user_id => $name)
                                    <input type="checkbox" name="usuarios[]" value="{{ $user_id }}"
                                        class="select-user" /> {{ $name }}<br>
                                @endforeach
